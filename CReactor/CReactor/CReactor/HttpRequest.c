...
void httpRequestReset(struct HttpRequest* request)
{
	...
	request->reqHeadersNum = 0;		// 标记无可用元素，下次使用从0号开始放
}
...
bool parseHttpRequestLine(struct HttpRequest* request, struct Buffer* readBuf)
{
	...
	if (lineSize)
	{
		...
		// 请求方式
		char* space = memmem(start, lineSize, " ", 1);		// 寻找从start开始sie大小，空格（1大小）的起始位置
		assert(space != NULL);
		int methodSize = space - start;
		request->method = (char*)malloc(methodSize + 1);	// +1是为了保存\0
		strncpy(request->method, start, methodSize);		// 将size大小的start字符串，复制到method中
		request->method[methodSize] = '\0';

		// 请求的静态资源
		start = space + 1;
		space = memmem(start, end - start, " ", 1);		// 寻找从start开始sie大小，空格（1大小）的起始位置
		assert(space != NULL);
		int urlSize = space - start;
		request->url = (char*)malloc(urlSize + 1);	// +1是为了保存\0
		strncpy(request->url, start, urlSize);		// 将size大小的start字符串，复制到method中
		request->url[urlSize] = '\0';

		// http版本
		start = space + 1;
		request->version = (char*)malloc(end - start + 1);	// +1是为了保存\0
		strncpy(request->version, start, end - start);		// 将size大小的start字符串，复制到method中
	...
	struct HttpResponse* response, struct Buffer* sendBuf, int socket)
{
	...
	request->curState = ParseReqLine;			// 状态还原，保证还能继续处理第二条及之后请求
	return flag;
}

// 处理http请求 -- 处理基于get的http请求
...
void sendDir(const char* dirName, struct Buffer* sendBuf, int cfd)
{
	...
	int num = scandir(dirName, &namelist, NULL, alphasort);		// 返回得到了多少个文件
	for (int i = 0; i < num; ++i)
	{
		...
		struct stat st;
		char subPath[1024] = { 0 };
		sprintf(subPath, "%s/%s", dirName, name);				// 注意：/表示从属关系
